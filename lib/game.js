...
function buildFloor(x1, z1, x2, z2) {
  ...
  let texture = new THREE.TextureLoader().load( "../assets/textures/floor_texture.jpg" );
...
function buildBall(radius) {
  let texture = new THREE.TextureLoader().load( "../assets/textures/ball.png" );
  let material	= new THREE.MeshPhongMaterial({
    map: texture
  });
...
function buildObstacleBall(radius, x, z) {
  let texture = new THREE.TextureLoader().load( "../assets/textures/obstacle_ball.png" );
  let material	= new THREE.MeshPhongMaterial({
    map: texture
  });
  let geometry	= new THREE.SphereGeometry(radius, 64, 64);
...
function buildObstacleCube(width, height, depth, x, z) {
  let texture = new THREE.TextureLoader().load( "../assets/textures/log_texture.jpg" );
  let geometry	= new THREE.CubeGeometry(width, height, depth);
  let material	= new THREE.MeshPhongMaterial({
    map: texture
  });
...
function buildWall(x1, z1, x2, z2){
  ...
  const depth	= Math.abs(z2 - z1);
  let geometry	= new THREE.CubeGeometry(width, height, depth);
  let texture = new THREE.TextureLoader().load( "../assets/textures/brick_texture.jpg" );
  let material	= new THREE.MeshPhongMaterial({
    map	: texture
  });
...
function buildPlane(width, height, x, z) {
