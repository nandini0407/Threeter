...
function updateBallPosition(keyboard, ball_body, nowMsec, deltaMsec) {
  ...
  // once key is pressed, we need to check if the ball is still on the floor,
  // that is, its y coordinate is in between -0.5 and 0.5. Otherwise, if there is
  // a forceful collision with another object, the ball can fly off the board
  if (keypressed === true &&
